...
# -*- coding: utf-8 -*-
"""
Created on Thur Jan 26 18:50:58 2022

Module for visualising Second Spectrum's Tracking data

@author: This code is originally written by Laurie Shaw (@EightyFivePoint) with slight amendments for Second Spectrum compatibility by Edd Webster (@eddwebster).
...
def plot_pitch( field_dimen = (106.0, 68), field_color ='white-smoke', linewidth=2, markersize=20):
    """ plot_pitch
    ...
    if field_color=='white-smoke':
        ax.set_facecolor('#F7F7F7')
        lc = '#3B3B3B' # line color
        pc = 'k' # 'spot' colors
    elif field_color=='city-blue':
...
def plot_frame( hometeam, awayteam, figax=None, team_colors=(colour_home, colour_away), field_dimen = (106.0, 68), include_player_velocities=False, PlayerMarkerSize=12, PlayerAlpha=0.7, annotate=False ):
...
def plot_frame(frame_idx,
               ...
               awayteam,
               figax=None,
               team_colors=(colour_home, colour_away),
    ...
    Plots a frame of Second Spectrum tracking data (player positions and the ball) on a football pitch. All distances should be in meters.
    ...
    ## Filter Tracking DataFrame for frame of interest
    hometeam = hometeam.loc[frame_idx]
    awayteam = awayteam.loc[frame_idx]
    ...
    ## Save figure
    if filepath != 'none':
        if not os.path.exists(f'{filepath}_{frame_idx}.png'):
            plt.savefig(f'{filepath}_{frame_idx}.png', dpi=300, format='png', transparent=False, bbox_inches='tight')
    else:
        pass
...
def save_match_clip(hometeam,awayteam, fpath, fname='clip_test', figax=None, frames_per_second=25, team_colors=(colour_home,colour_away), field_dimen = (106.0, 68), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7):
    """ save_match_clip( hometeam, awayteam, fpath )
    
    Generates a movie from Second Spectrum tracking data, saving it in the 'fpath' directory with name 'fname'
    ...
    metadata = dict(title='Tracking Data', artist='Matplotlib', comment='Second Spectrum tracking data clip')
...
def plot_events(events, figax=None, field_dimen = (106.0, 68), indicators = ['Marker','Arrow'], color='r', marker_style = 'o', alpha = 0.5, annotate=False):
    ...
    Plots Second Spectrum event positions on a football pitch. event data can be a single or several rows of a data frame. All distances should be in meters.
...
def plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF, cmap='RdYlGn', alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0, 68)):
    ...
    -----------
        ...
        PPCF: Pitch control surface (dimen (n_grid_cells_x,n_grid_cells_y) ) containing pitch control probability for the attcking team (as returned by the generate_pitch_control_for_event in Second_Spectrum_PitchControl)
...
def plot_pitchcontrol_for_frame(frame,  tracking_home, tracking_away, PPCF, xgrid, ygrid, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (106.0,68), team_colors=(colour_home, colour_away), team_pass='Home'):
    ...
    -----------
        ...
        tracking_away: (entire) tracking DataFrame for the Away team
        PPCF: Pitch control surface (dimen (n_grid_cells_x,n_grid_cells_y) ) containing pitch control probability for the attcking team (as returned by the generate_pitch_control_for_event in Second_Spectrum_PitchControl)
        xgrid: Positions of the pixels in the x-direction (field length) as returned by the generate_pitch_control_for_event in Second_Spectrum_PitchControl
        ygrid: Positions of the pixels in the y-direction (field width) as returned by the generate_pitch_control_for_event in Second_Spectrum_PitchControl
        alpha: alpha (transparency) of player markers. Default is 0.7
        include_player_velocities: Boolean variable that determines whether player velocities are also plotted (as quivers). Default is False
        annotate: Boolean variable that determines with player jersey numbers are added to the plot (default is False)
        field_dimen: tuple containing the length and width of the pitch in meters. Default is (106,68)
...
def plot_EPV_for_event(event_id, events, tracking_home, tracking_away, PPCF, EPV, alpha = 0.7, include_player_velocities=True, annotate=False, autoscale=0.1, contours=False, field_dimen = (106.0, 68)):
    ...
    -----------
        ...
        tracking_home: (entire) tracking DataFrame for the Home team
        tracking_away: (entire) tracking DataFrame for the Away team
        PPCF: Pitch control surface (dimen (n_grid_cells_x,n_grid_cells_y) ) containing pitch control probability for the attcking team (as returned by the generate_pitch_control_for_event in Second Spectrum_PitchControl)
        EPV: Expected Possession Value surface. EPV is the probability that a possession will end with a goal given the current location of the ball. 
             The EPV surface is saved in the FoT github repo and can be loaded using Second Spectrum_EPV.load_EPV_grid()
        alpha: alpha (transparency) of player markers. Default is 0.7
        include_player_velocities: Boolean variable that determines whether player velocities are also plotted (as quivers). Default is False
        annotate: Boolean variable that determines with player jersey numbers are added to the plot (default is False)
...
def plot_EPV(EPV,field_dimen=(106.0,68),attack_direction=1):
    ...
    -----------
        EPV: The 32x50 grid containing the EPV surface. EPV is the probability that a possession will end with a goal given the current location of the ball. 
             The EPV surface is saved in the FoT github repo and can be loaded using Second_Spectrum_EPV.load_EPV_grid()
        field_dimen: tuple containing the length and width of the pitch in meters. Default is (106,68)
